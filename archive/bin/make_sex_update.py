...
FID is set to match IID (consistent with --double-id used in VCF_TO_PLINK),
so that PLINK's --update-sex lookup succeeds.
...
with open(infile) as fh, open(outfile, "w") as out:
    ...
    for line in fh:
        ...
        # FID = IID to match --double-id convention used in VCF_TO_PLINK
        out.write(iid + "\t" + iid + "\t" + plink_sex + "\n")